...
while(True):
    ...
    dist=math.sqrt((apple_coord_x - hand_coord_x)**2 + (apple_coord_y - hand_coord_y)**2)
    ...
    if(dist < 40 and dist > 1):
        img.draw_string(0, 120, "Grab", color=(255, 255, 0), scale = 2)
        print("Grabbing..")
        try:
            client.connect()
            client.publish("openmv/test", "Grab")
            client.disconnect()
        except Exception as e:
            print("Failed to connect MQTT Error: ", e)

    else:
        img.draw_string(0, 140, "release", color=(0, 255, 255), scale = 2)
        print("Releasing..")
        try:
            client.connect()
            client.publish("openmv/test", "Release")
            client.disconnect()
        except Exception as e:
            print("Failed to connect MQTT Error: ", e)
